...
#-------------------------------------------------------------------------------
# Filter to keep up to 95th percentile of data
week_df <- lapply(week_dat, function(dat) {
  dat %>%
    filter(
      across(2:5, ~ .x <= quantile(.x, 0.95, na.rm = TRUE)) |> 
        reduce(`&`)
    )
})
...
results_list <- fit_sine_to_list(week_df)
...
fwrite(df_unique,"c:/Users/maria/Desktop/Research/2024/microclimate/processed_data/merged_sample_wk_amplitude_df_10.9.25.csv")
